...
class ProjectSettingsDialog(QDialog):
    ...
    def __init__(self, manager: ProjectManager, parent: QWidget = None):
        ...
        self._stored = self._read_stored_data()
        ...
        # The baseline is read back off the widgets rather than from the database,
        # so _is_dirty() compares like with like. Comparing widget values against
        # raw rows made the two shapes differ permanently — the rows carry an id
        # column and store purity as text, the widgets add a species field and a
        # date default — so the dialog reported unsaved changes every time it was
        # dismissed, whether or not anything had been touched.
        self.data_snapshot = self._gather_ui_data()
    ...
    def _init_ui(self) -> None:
        ...
        substance_group = CollapsibleSection("Substance Parameters")
        substance_body = QWidget()
        substance_layout = QGridLayout(substance_body)
        ...
        substance_group.setContentLayout(substance_body)
        content_layout.addWidget(substance_group)


        # Section 3: Water Quality & Conditions (Collapsible) 
        conditions_group = CollapsibleSection("Water Quality & Test Conditions")
        conditions_body = QWidget()
        conditions_layout = QGridLayout(conditions_body)
        ...
        self.fertilization_input = QLineEdit(); self.fertilization_input.setToolTip("Overall fertilization rate of the egg batch, scored before plating (e.g., '88 %'). OECD TG 236 requires ≥ 70%. Leave blank if not recorded.")
        ...
        conditions_layout.addWidget(QLabel("Batch Fertilization Rate:"), 8, 0); conditions_layout.addWidget(self.fertilization_input, 8, 1)
        conditions_group.setContentLayout(conditions_body)
        content_layout.addWidget(conditions_group)

        # Section 4: Test Organisms (Collapsible) 
        organisms_group = CollapsibleSection("Test Organisms")
        organisms_body = QWidget()
        organisms_layout = QGridLayout(organisms_body)
        ...
        self.species_input = QLineEdit(); self.species_input.setToolTip("Scientific name of the test species (e.g., 'Danio rerio').")
        organisms_layout.addWidget(QLabel("Species:"), 0, 0); organisms_layout.addWidget(self.species_input, 0, 1)
        ...
        organisms_group.setContentLayout(organisms_body)
        content_layout.addWidget(organisms_group)
        
        # Section 5: Methodology (Collapsible) 
        methodology_group = CollapsibleSection("Methodology")
        methodology_body = QWidget()
        methodology_layout = QGridLayout(methodology_body)
        ...
        methodology_group.setContentLayout(methodology_body)
        ...
        # Bottom Buttons — a QDialogButtonBox so the platform decides the order
        # and the standard roles carry the keyboard defaults.
    ...
    def _read_stored_data(self) -> Dict[str, Any]:
        """Project data as held in the database, used to populate the fields."""
    ...
    def _populate_fields(self) -> None:
        ...
        # General Info
        self.name_input.setText(self._stored["project_name"])
        self.researcher_input.setText(self._stored["user"])
        self.conc_unit_input.setText(self._stored["concentration_unit"])
        self.start_date_input.setDate(QDate.fromString(self._stored["start_date"], Qt.ISODate))
        self.days_input.setValue(self._stored["num_days"])
        self.substance_input.setText(self._stored["substance"])
        
        # Substance Details
        substance_details = self._stored["substance_details"]
        ...
        # Test Conditions
        test_conditions = self._stored["test_conditions"]
        ...
        self.fertilization_input.setText(test_conditions.get("fertilization_rate", ""))
        ...
        # Test Organisms
        test_organisms = self._stored["test_organisms"]
        self.species_input.setText(test_organisms.get("species", DEFAULT_SPECIES))
        ...
        # Methodology
        methodology = self._stored["methodology"]
        ...
        # Notes
        self.notes_input.setText(self._stored["report_notes"])
